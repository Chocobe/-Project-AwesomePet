const replyContainer = $(".petContentsWrap .replyContainer");
const replyPageContainer = $(".petContentsWrap .replyPageContainer");
...
// 댓글을 조회합니다.
	...
	function printReply(parsedJSON) {
		const petReplyContentsList = parsedJSON.petReplyContentsList;
		
		// 출력하기 위한 태그요소들을 생성합니다.
		for(let i in petReplyContentsList) {
			...
			const replyIDX = $("<input>").attr({
				"type": "hidden", 
				"class": "replyIDX",
				"value": petReplyContentsList[i].replyIDX
			});
			...
			// 5. 작성자
			const writerID = $("<p>");
			writerID.text(petReplyContentsList[i].writerID);
			
			// 6. 작성일
			const writeDate = $("<p>");
			const currentDate = petReplyContentsList[i].writeDate;
			writeDate.text("(" + currentDate.year + "-" + currentDate.month + "-" + currentDate.day + ")");
			
			// 7. 수정버튼
			const fixButton = $("<input>").attr({
				"type": "button", 
				"value": "🔧", 
				"onclick": "changeFormUpdateReply(this);"
			});
			...
			if(memberLoginID == petReplyContentsList[i].writerID) {
				replyTitleContainer.append(fixButton);
				replyTitleContainer.append(deleteButton);
			}
			
			// 10. 개행문자 단위로 분할하여 출력합니다.
			const splitedContents = petReplyContentsList[i].content.trim().split("\n");
	...
	function printReplyPage(parsedJSON) {
		...
		// 1. 댓글 "시작" 링크
		const firstPage = $("<a>").attr({
			"onclick": "loadReply(`${contextPath}`, `${parentIDX}`, `1`, `${memberLoginID}`);"
		});
		...
		// 2. 댓글 "이전" 링크
		const beginPage = $("<a>").attr({
			"onclick": "loadReply(`${contextPath}`, `${parentIDX}`, " + parsedJSON.prevPage + ", `${memberLoginID}`);"
		});
		...
		for(let i = parsedJSON.beginPage; i <= parsedJSON.endPage; i++) {
			...
			if(i == parsedJSON.currentPage) {
				...
				currentPage = $("<a>").attr({
					"onclick": "loadReply(`${contextPath}`, `${parentIDX}`, " + i + ", `${memberLoginID}`);"
				});
		...
		// 4. 댓글 "다음" 링크	
		const nextPage = $("<a>").attr({
			"onclick": "loadReply(`${contextPath}`, `${parentIDX}`, " + parsedJSON.nextPage + ", `${memberLoginID}`);"
		});
		...
		// 5. 댓글 "끝" 링크
		const lastPage = $("<a>").attr({
			"onclick": "loadReply(`${contextPath}`, `${parentIDX}`, " + parsedJSON.totalPageCnt + ", `${memberLoginID}`);"
		});
	...
	function petReplyWrite() {
		...
		if(inputValue.trim() == "") {
			...
			$.ajax({
				type: "POST",
				async: true,
				url: contextPath + "/petReplyWrite.do",
				data: {
					"parentIDX": parentIDX,
...
// 댓글을 수정합니다.
	...
	function changeFormUpdateReply(target) {
		...
		const replyWriteContainer = $("<div>").attr({
			"class": "replyWriteContainer"
		});
		
		const replyTextarea = $("<textarea>").attr({
			"class": "replyTextarea"
		});
		
		const replyButtonContainer = $("<div>").attr({
			"class": "replyButtonContainer"
		});
	...
	// 수정한 댓글을 갱신합니다.
	function petReplyUpdate(target) {
		...
		$.ajax({
			type: "POST",
			async: true,
			url: contextPath + "/petReplyUpdate.do",
			datatype: "TEXT",
			data: {
				"replyIDX": replyIDX,
				"content": inputValue
...
// 댓글을 삭제 합니다.
	function petReplyDelete(target) {
		const replyIDX = $(target).parent().parent().children(".replyIDX")[0].value;
		
		location.href = contextPath + "/petReplyDelete.do?parentIDX=" + parentIDX + "&requestReplyIDX=" + replyIDX;
	}