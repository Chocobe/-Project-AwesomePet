const replyContainer = $(".communicationContentsWrap .replyContainer");
const replyPageContainer = $(".communicationContentsWrap .replyPageContainer");
...
// 댓글을 조회합니다.
	function loadReply(context, parent, replyPage, loginID) {
		$.ajax({
			type: "GET",
			async: true,
			url: context + "/communicationReplyView.do",
			datatype: "TEXT",
			data: {
				"parentIDX": parent,
	...
	function printReply(parsedJSON) {
		const communicationReplyContentsList = parsedJSON.communicationReplyContentsList;
		
		// 출력하기 위한 태그요소들을 생성합니다.
		for(let i in communicationReplyContentsList) {
			...
			const replyIDX = $("<input>").attr({"type": "hidden", "class": "replyIDX" ,"value": communicationReplyContentsList[i].replyIDX});
			...
			// 5. 작성자
			const writerID = $("<p>");
			writerID.text(communicationReplyContentsList[i].writerID);
			
			// 6. 작성일
			const writeDate = $("<p>");
			const currentDate = communicationReplyContentsList[i].writeDate;
			...
			// 8. 삭제버튼
			const deleteButton = $("<input>").attr({"type": "button", "value": "❌", "onclick": "communicationReplyDelete(this);"});
			...
			if(memberLoginID == communicationReplyContentsList[i].writerID) {
				replyTitleContainer.append(fixButton);
				replyTitleContainer.append(deleteButton);
			}
			
			// 10. 개행문자 단위로 분할하여 출력합니다.
			const splitedContents = communicationReplyContentsList[i].content.trim().split("\n");
...
// 댓글을 작성합니다.
	function communicationReplyWrite() {
		...
		if(inputValue.trim() == "") {
			...
			$.ajax({
				type: "POST",
				async: true,
				url: contextPath + "/communicationReplyWrite.do",
				data: {
					"parentIDX": parentIDX,
...
// 댓글을 수정합니다.
	...
	function changeFormUpdateReply(target) {
		...
		const updateButton = $("<input>").attr({"type": "button", "class": "replyButton", "value": "수정하기", "onclick": "communicationReplyUpdate(this);"});
	...
	// 수정한 댓글을 갱신합니다.
	function communicationReplyUpdate(target) {
		...
		$.ajax({
			type: "POST",
			async: true,
			url: contextPath + "/communicationReplyUpdate.do",
			datatype: "TEXT",
			data: {
				"replyIDX": replyIDX,
				"content": inputValue
...
// 댓글을 삭제 합니다.
	function communicationReplyDelete(target) {
		const replyIDX = $(target).parent().parent().children(".replyIDX")[0].value;
		
		location.href = contextPath + "/communicationReplyDelete.do?parentIDX=" + parentIDX + "&requestReplyIDX=" + replyIDX;
	}